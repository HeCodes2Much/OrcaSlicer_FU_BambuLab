    ...
    ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ...
    if (!orca_lib_vendor.empty()) {
        try {
            boost::filesystem::path orca_lib_dir = dir;
            if (!validation_mode) {
                const boost::filesystem::path resources_profiles_dir = (boost::filesystem::path(resources_dir()) / "profiles")
                                                                           .make_preferred();
                if (boost::filesystem::exists(resources_profiles_dir / (orca_lib_vendor + ".json")) &&
                    ...
                    orca_lib_dir = resources_profiles_dir;
                    BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ": load OrcaFilamentLibrary from resources: " << orca_lib_dir.string();
                }
            }
            append(substitutions,
                   this->load_vendor_configs_from_json(orca_lib_dir.string(), orca_lib_vendor, PresetBundle::LoadSystem, compatibility_rule)
                       .first);
            first = false;
        } catch (const std::runtime_error& err) {
            if (validation_mode)
